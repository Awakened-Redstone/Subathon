...
import old.Subathon;
import old.client.screen.EventLogScreen;
import old.commands.SubathonCommand;
import old.events.HudRenderCallback;
import old.renderer.PositionedText;
...
import old.util.ConversionUtils;
    ...
    public void onInitializeClient() {
        ...
        HudRenderCallback.PRE_TICK.register(paused -> {
            if (showData) {
                ...
                float x = ConversionUtils.toFloat(client.getWindow().getScaledWidth() - (client.textRenderer.getWidth(message) * fontScale) - 4);
                ...
                if (Subathon.getConfigData().showResetTimer && resetTimer != 0 && serverTicks != 0) {
                    ...
                    x = ConversionUtils.toFloat(client.getWindow().getScaledWidth() - (client.textRenderer.getWidth(message) * (fontScale / 2)) - 4);
