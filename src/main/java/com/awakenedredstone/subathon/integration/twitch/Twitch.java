...
public final class Twitch {
    ...
    @Getter private final TwitchClientPool clientPool;
    @Getter private final TwitchChatConnectionPool chatPool;
    @Getter private final TwitchEventSocketPool eventSub;
    ...
    public Twitch() {
        ...
        eventSub = isDev ? eventSubBuilder.baseUrl("ws://localhost:3000").helix(helix).build() : eventSubBuilder.build();
    ...
    public void disconnectIRC(UUID uuid) {
        ...
        ServerPlayerEntity player = Subathon.getServer().getPlayerManager().getPlayer(uuid);
    ...
    public void disconnectEventSub(String token) {
        new Thread(() -> {
            if (token != null && subscriptions.containsKey(token)) {
                ...
                ServerPlayerEntity player = Subathon.getServer().getPlayerManager().getPlayer(uuidOptional.orElse(null));
